...
const Cart = ({ cardata, setcarddata }) => {
  ...
  // Individual Remove Function
  const removeFromCart = (id) => {
    setcarddata(prev => prev.filter(item => item.id !== id));
    toast.error("Item Removed from Cart", { position: "top-right", autoClose: 1000 });
  };
  ...
  if (isOrderPlaced) return (
    <div className="text-center py-40">
      ...
      <p className="text-gray-500 mt-5">Redirecting you back shortly...</p>
    </div>
  );
      ...
      <div className="bg-white rounded-[45px] border-2 border-cyan-300 p-10 shadow-2xl">
        ...
        {cardata.length === 0 ? (
          ...
          <div className="space-y-6">
            {cardata.map((item) => (
              <div key={item.id} className="flex items-center justify-between p-6 bg-gray-50 rounded-[30px] border border-gray-100">
                ...
                <button 
                  ...
                  className="text-red-500 font-bold hover:bg-red-50 px-5 py-2 rounded-full transition-all"
                >
                  Remove
                </button>
              </div>
            ...
            <div className="flex justify-between items-center pt-10 border-t">
              <span className="text-gray-400 text-xl font-bold">Total Payable:</span>
              <span className="text-5xl font-black text-gray-900">${total}</span>
            </div>

            <button 
              onClick={handleCheckout}
              className="w-full bg-[#7C3AED] text-white font-bold py-6 rounded-full text-2xl mt-8 hover:bg-[#6D28D9] transition-all shadow-xl shadow-purple-100"
            >
              Checkout Now
            </button>
          </div>
